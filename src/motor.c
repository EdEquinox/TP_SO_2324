...
void usersCommandCurses() // Função que trata do comando users.
{
  wprintw(janelaComandos,"\nComando [users] nao implementado.\n");
}

void kickCommandCurses(char *username) // Função que trata do comando kick.
{
  wprintw(janelaComandos,"\nComando [kick] nao implementado.\n");
}

void botsCommandCurses() // Função que trata do comando bots.
{
  wprintw(janelaComandos,"\nComando [bots] nao implementado.\n");
}

void bmovCommandCurses() // Função que trata do comando bmov.
{
  wprintw(janelaComandos,"\nComando [bmov] nao implementado.\n");
}

void rbmCommandCurses() // Função que trata do comando rbm.
{
  wprintw(janelaComandos,"\nComando [rbm] nao implementado.\n");
}

void beginCommandCurses() // Função que trata do comando begin.
{
...
void handler_saSIGWINCH(int sigNum, siginfo_t *info, void *old) {
  ...
  mvprintw(1, 10, "____________________________________________________");  // mensagem fora da janela, na linha 1, coluna 10 do ecrã
  mvprintw(2, 10, "[ space - muda para o teste de comandos / q - sair ]"); // mensagem fora da janela, na linha 2, coluna 10 do ecrã
  mvprintw(3, 10, "____________________________________________________"); // mensagem fora da janela, na linha 3, coluna 10 do ecrã
...
void readMap(int level) { // Função que lê o mapa do ficheiro.
  char filename[20];
...
void desenhaJanela(WINDOW *janela, int tipo) // Função que desenha o mapa.
{
  if (tipo == 1)
  {
...
void trataTeclado() // Função que trata do teclado.
{
  curs_set(0); // faz com que o cursor não seja visivel
  int tecla = wgetch(janelaMapa); // MUITO importante: o input é feito sobre a janela em questão, neste caso na janelaMapa
  ...
  while (tecla != 113) // trata as tecla até introduzirem a letra q. O código asci de q é 113
  {
    ...
    if (tecla == ' ') // trata a tecla espaço
    {
      ...
      wgetstr(janelaComandos, command);  // para receber do teclado uma string na "janelaComandos" para a variavel comando
