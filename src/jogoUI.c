...
void handler_saSIGWINCH(int sigNum, siginfo_t *info, void *old) {
  ...
  mvprintw(2, 10, "[                     Bem-vindo                    ]"); // mensagem fora da janela, na linha 2, coluna 10 do ecrã
  mvprintw(3, 10, "[                   Testa as setas!                ]"); // mensagem fora da janela, na linha 3, coluna 10 do ecrã
  mvprintw(4, 10, "[ space - muda para o teste de comandos / q - sair ]"); // mensagem fora da janela, na linha 4, coluna 10 do ecrã
  mvprintw(5, 10, "____________________________________________________"); // mensagem fora da janela, na linha 5, coluna 10 do ecrã
...
void trataTeclado()
{
  ...
  int tecla = wgetch(janelaMapa); // MUITO importante: o input é feito sobre a janela em questão, neste caso na janelaTopo
  ...
  while (tecla != 113) // trata as tecla até introduzirem a letra q. O código asci de q é 113
  {
    ...
    if (tecla == KEY_UP) // quando o utilizador introduz a seta para cima
    {
      ...
      wrefresh(janelaMapa);// // necessário para atualizar a janelaTopo. Nota: é apenas esta janela que pretendemos atualizar
    }
...
void playersCommand() // Comando players
{
  wprintw(janelaComandos,"\nComando [players] nao implementado.\n");
}

void msgCommand(char *username, char* msg) // Comando msg
{
  wprintw(janelaComandos,"\nComando [msg] nao implementado.\n");
}

void exitCommand() // Comando exit
{
